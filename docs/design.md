...
```
ququer [--config-dir <path>] <COMMAND>
...
默认路径：`~/.ququer/config.toml`（可通过 `--config-dir` 覆盖）
...
其他本地文件（均在 config-dir 下）：
- `keys.json` — Ed25519 密钥对（hex 编码）
- `token.json` — 缓存的 session token

多 agent 运行：`ququer --config-dir ~/.ququer-agent-a register --name agent-a`
...
src/
  ...
  config.rs         配置加载（--config-dir 或 ~/.ququer）
...
### config.rs
加载 config-dir 下的 `config.toml`，不存在则用默认值。自动创建目录。默认 `~/.ququer/`，可通过 `--config-dir` 覆盖。
...
ququer submit <game_id> '{"action":"rock"}'
  ...
  → [simultaneous + CR]:
      ...
      输出 phase_result
  → [sequential]:
      从 data 中提取 actionType（data.type 字段）
      crypto::sign_message(key, "actionType:JSON(data)")
      SSE /api/sse/game/{id} 先建立连接（避免错过事件）
      sse::spawn_heartbeat(game_id)
      POST /api/game/{id}/action { actionType, data, signature }
      SSE 等待 phase_result（按 phase ID 过滤）
      停止心跳
      输出 phase_result
```
